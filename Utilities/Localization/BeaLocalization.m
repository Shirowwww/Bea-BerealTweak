...
static NSDictionary<NSString *, NSDictionary<NSString *, NSString *> *> *BeaOwnStrings(void) {
	...
	dispatch_once(&onceToken, ^{
		strings = @{
			...
			@"settings.gating_hide_detail": @{ @"en": @"Removes the lock overlay drawn over a gated post. BeReal draws it with no view of its own, so this falls back to hiding the drawing layers stacked over the photo - turn it off if anything else on a post disappears.",
			                                   @"fr": @"Retire le voile affiché par-dessus un post verrouillé. BeReal le dessine sans vue propre : le tweak masque alors les calques posés sur la photo. Désactive si autre chose disparaît d’un post." },
			...
			@"settings.button_download_detail": @{ @"en": @"One per post on screen, in the corner of its own photo.",
			                                       @"fr": @"Un par post affiché, dans le coin de sa propre photo." },
			@"settings.button_upload_detail": @{ @"en": @"Long-press it to open these settings. With it off, long-press the download button, or hold two fingers anywhere.",
			                                     @"fr": @"Appuie longuement dessus pour ouvrir ces réglages. Désactivé, appuie longuement sur le bouton de téléchargement, ou pose deux doigts n’importe où." },
			@"settings.button_hide_scrolling": @{ @"en": @"Fade out while scrolling", @"fr": @"Estomper pendant le défilement" },
			@"settings.button_hide_scrolling_detail": @{ @"en": @"Applies to every button this tweak adds. Off keeps them all pinned in place at all times.",
			                                             @"fr": @"S’applique à tous les boutons ajoutés par le tweak. Désactivé, ils restent tous fixes en permanence." },
